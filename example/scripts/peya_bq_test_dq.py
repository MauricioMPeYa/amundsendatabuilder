...
from databuilder.extractor.bigquery_peya_dataquality_extractor import *
...
def run_bq_dq_job(job_name):
    ...
    job_config = ConfigFactory.from_dict({
            'extractor.bigquery_table_metadata.{}'.format(
            BigQueryPeyaDQExtractor.PROJECT_ID_KEY
            ): gcloud_project,
        'loader.filesystem_csv_neo4j.node_dir_path': node_files_folder,
        'loader.filesystem_csv_neo4j.relationship_dir_path': relationship_files_folder,
        'loader.filesystem_csv_neo4j.delete_created_directories': True,
    ...
    task = DefaultTask(extractor=BigQueryPeyaDQExtractor(),
                       loader=FsNeo4jCSVLoader(),
                       transformer=NoopTransformer())
...
#    if create_connection(DB_FILE):
    ...
    print("EMPIEZA A CORRER EL JOB DE TABLE DATA QUALITY...")

    start_time_last_upd = time.time()

    run_bq_dq_job("test_bq_lu")


    
    print("TERMINA DE CORRER EL JOB DE TABLE TABLE DATA QUALITY...")
    print("--- tiempo total de ejecucion de carga de table data quality neo4j %s seconds ---" % (time.time() - start_time_last_upd))
